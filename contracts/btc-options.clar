...
(define-private (update-user-balance
    ...
    (let (
            ...
            (new-sbtc (if is-subtract
                (begin
                    (asserts! (>= sbtc delta) ERR_INSUFFICIENT_BALANCE)
                    (- sbtc delta)
                )
                (+ sbtc delta)
            ))
        )
        (ok (map-set user-balances user (merge bal { sbtc-balance: new-sbtc })))
    )
)
...
(define-public (create-option
    ...
    (let (
        ...
        (asserts! (>= (get sbtc-balance user-bal) required-collateral)
